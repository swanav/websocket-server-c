...
static void onMessage(Endpoint_t* client, void* payload, size_t size) {
	...
	if(message->opcode == OP_TEXT) {
        printf("Broadcasting Text Message: %.*s\n", (int) message->length, message->buffer);
    } else {
        printf("Broadcasting Binary Message: ");
        for(size_t i = 0; i < message->length; i++) {
            printf("%02X ", message->buffer[i]);
        }
        printf("\n");
    }
    // Broadcast the message to all clients connected to the server.
    ws_server_BroadcastMessage((WsServer_t*) client->server, message->opcode, message->buffer, message->length);
}
...
int main() {
    ...
    printf("Websocket Secure Broadcast Server listening on Port: %d\n", PORT);
